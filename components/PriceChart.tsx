...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    const pricePayload = payload.find((p: any) => p.dataKey === 'price1' || p.dataKey === 'price2');
    ...
    return (
      <div className="bg-slate-800 border border-slate-700 p-3 rounded shadow-lg text-sm text-right min-w-[180px]" dir="rtl">
        ...
        <div className="flex justify-between items-center gap-4 mb-2">
          <span className="text-emerald-400 font-medium">قیمت:</span>
          <span className="font-bold text-white">
            {new Intl.NumberFormat('fa-IR').format(priceValue)}
          </span>
        </div>
          ...
          <div className="mb-2 bg-slate-700/30 p-2 rounded">
            <div className="flex justify-between items-center gap-4 mb-1">
              ...
              <span className="font-bold text-slate-200 text-xs">
                {new Intl.NumberFormat('fa-IR', { maximumFractionDigits: 0 }).format(ma100Value)}
              </span>
            </div>
        ...
        {ma200Value && (
          <div className="bg-slate-700/30 p-2 rounded">
            <div className="flex justify-between items-center gap-4 mb-1">
              ...
              <span className="font-bold text-slate-200 text-xs">
                {new Intl.NumberFormat('fa-IR', { maximumFractionDigits: 0 }).format(ma200Value)}
              </span>
            </div>
            <div className="flex justify-between items-center gap-4">
              <span className="text-slate-500 text-[10px]">فاصله:</span>
