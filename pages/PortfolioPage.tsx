...
export function PortfolioPage() {
  ...
  const calculateStateHysteresis = (data: TsetmcDataPoint[], maMap: Map<string, number>, targetIndex: number) => {
    ...
    return { state: currentState as MarketState, stateSince: toShamsi(stateStartDate), daysActive: targetIndex - data.findIndex(d => d.date === stateStartDate) + 1, timer, timerType };
  };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-8 pb-20 animate-fade-in">
       <header>
          <h2 className="text-4xl font-black text-white mb-2">«دستیار هوشمند سبدگردانی»</h2>
          <p className="text-slate-400 font-medium">«پیشنهاد تخصیص دارایی»</p>
       </header>
